...
Level.prototype.obstacleAt = function (pos, size) {
  ...
  //Sides top and bottom of level as walls
  if (xStart < 0 || xEnd > this.width || yStart < 0  || yEnd > this.height)
    return "wall";

  //Check grid position starting at ystart and xStart for possible obstravles (non null value)
...
Player.prototype.moveY = function(step, level, keys) {
  //Accelerate polayer downwards always
  ...
  //Floor is also an obstacle  - only allows player to jump if they are touching some obstacle
  if (obstacle) {
